export type PriorityQueueComparator<TValue> = (left: TValue, right: TValue) => number;

export interface PriorityQueueOptions<TValue> {
  readonly compare: PriorityQueueComparator<TValue>;
}
...
export class PriorityQueue<TValue> {
  ...
  public constructor(private readonly options: PriorityQueueOptions<TValue>) {
    PriorityQueue.ensureComparator(options.compare);
  }
  ...
  public extract(): TValue | undefined {
  ...
  public getStats(): PriorityQueueStats {
  ...
  private static ensureComparator<TValue>(
    comparator: PriorityQueueComparator<TValue> | undefined,
  ): asserts comparator is PriorityQueueComparator<TValue> {
