...
const ROLLBACK_SIGNAL = 'temporary-fact-visibility-rollback';
...
runCli(async () => {
  ...
  const targetTraceId = args['target-trace'] ?? `temporary-fact-replay-${clusterKey}-${asOf.toISOString().slice(0, 10)}`;
  const factSource = args['fact-source'] ?? 'historical_limitup_news';
  ...
  try {
    await prisma.$transaction(async (tx: any) => {
      const [visibleFacts, visibleAliases] = await Promise.all([
        tx.stockExposureFact.findMany({
          where: {
            clusterKey,
            source: factSource,
            status: 'active',
      ...
      const factIds = visibleFacts.map((row: { id: string }) => row.id);
      const aliasIds = visibleAliases.map((row: { id: string }) => row.id);
      if (factIds.length > 0) {
        await tx.stockExposureFact.updateMany({
          where: { id: { in: factIds } },
          data: { validFrom: asOf },
        });
      }
      ...
      const [recommendations, limitUpCases, factContributionMethods, aliasContributionMethods] = await Promise.all([
      ...
      summary = {
        ...
        copiedSignalCount,
        copiedGraph,
        temporarilyVisibleFactCount: factIds.length,
    ...
    }, { timeout: readNumberArg(args['transaction-timeout-ms'], 60_000) });
  }
  catch (error) {
  ...
  writeJson({
    rolledBack: true,
    rollbackReason: 'temporary fact visibility is only for retrospective what-if replay',
    ...summary,
  });
});
